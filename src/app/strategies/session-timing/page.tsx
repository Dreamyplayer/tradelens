  ...
  return (
    ...
    >
      <Section label="WHAT" title="What session timing actually affects">
        <p>
          None of the four strategies change — S/R zones, trend pullbacks,
          breakouts, and order blocks are all still valid setups at any
          hour. What changes is <strong>how reliable</strong> the resulting
          move is, because volume (the actual fuel behind a breakout or a
          zone rejection) isn't evenly spread across the day.
        </p>
      ...
      <Section label="HOW" title="How to use this practically">
        <p>
          Keep a simple internal map of your instrument's session: mark the
          open, the typical lull, and the highest-volume overlap window on
          your own trading hours. Weight your setups accordingly — a
          breakout during the lull needs a much higher bar of confirmation
          than the same pattern at the open or the overlap.
        </p>
      </Section>

      <Section label="WHY" title="Why this matters">
        <p>
          Every strategy in this guide is ultimately about reading order
          flow — where real orders are clustering and firing. When overall
          volume is thin, there simply are fewer real orders behind any
          given move, so patterns that look identical on the chart carry
          different odds of following through depending on the hour they
          form in.
        </p>
      </Section>
